...
Phase 3 Public Survey/Response는 contract 기준으로 승인됐다. Phase 3-A의 exact anonymous `GET /api/public/surveys/{slug}`와 respondent-safe read DTO는 `dev`에 통합됐다. 현재 tree는 Phase 3-B V6 Answer schema, caller-owned Response/Answer persistence와 deterministic canonical payload hash를 구현하며, user merge와 latest `dev` validation 전에는 3-C를 시작하지 않는다. 이후 Product 구현은 3-C Atomic Public Submission Backend → 3-D Respondent Frontend 순서의 별도 Issue/PR로 진행한다. Public Response POST와 `/s/:slug` route는 아직 없고 Phase 4 Result/CSV와 Production도 계속 승인되지 않았다.
