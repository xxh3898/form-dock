...
Phase 3 Public Survey/Response는 implementation contract 기준으로 승인됐다. Phase 3-A anonymous OPEN Public Survey GET은 `dev`에 통합됐고, 현재 tree는 Phase 3-B V6/data/canonicalization foundation을 구현한다. user merge와 latest `dev` validation 전에는 3-C를 시작하지 않으며 이후 3-C→3-D도 별도 Issue/PR로 직렬 진행한다. Public Response POST와 `/s/:slug`는 아직 없고 Phase 4 Result/CSV와 Production도 계속 승인되지 않았다.
