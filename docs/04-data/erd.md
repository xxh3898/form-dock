...
현재 shared Flyway history는 V1~V5이며 immutable하다. Phase 3-B의 next migration은 future `V6__create_answers_and_answer_options.sql` 하나로 `answers`와 `answer_options`만 생성한다. Existing V5 `survey_responses`는 변경하거나 다시 생성하지 않는다.
