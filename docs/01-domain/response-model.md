...
SurveyResponse row creation, canonical payload hash/idempotency runtime과 Public Response submission은 Phase 3가 처음 소유한다. Phase 3 Entry는 이 contract만 승인하며 runtime은 별도 3-A→3-D Issue/PR에서 직렬 구현한다. 기존 V1~V5 migration은 immutable하고, Phase 3-B가 future `V6__create_answers_and_answer_options.sql`로 `answers`와 `answer_options`만 추가한다. Phase 2/3 모두 temporary row, `structure_locked` flag 또는 denormalized response count를 별도 authority로 만들지 않는다.
