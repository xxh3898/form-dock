...
## Production CD Foundation and Control Plane
...
Issue #103의 별도 Production Operations Gate는 CD foundation을 `main`에 release하고, exact `main` 전용 `Production` Environment, environment-scoped deploy credential, restricted SSH, stable runtime과 initial successful Production baseline을 구성했다. `MAC_MINI_DEPLOY_ENABLED=true` 이후 exact current-main safe no-op dispatch가 validation만 성공하고 publication/deploy를 건너뛴 것을 확인했다. 따라서 control plane은 `ACTIVE + ACCEPTED`, automation은 `ARMED`이며, 첫 eligible application release의 실제 자동 배포는 아직 pending이다. 현재 v0.4.0 application과 PostgreSQL volume/Flyway는 보존됐다. 상세 근거는 [Production CD Control-plane Activation Evidence](../06-quality/phase-5-production-cd-control-plane-activation-evidence.md)를 따른다.
