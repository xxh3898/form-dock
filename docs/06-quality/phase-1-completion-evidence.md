...
origin/dev   9d29f47165968b0308ea7b3e90c2183fe68cc48c
```

`main...dev` release 범위에는 first-parent merge boundary 9개와 변경 파일 119개, 9,892 insertions, 237 deletions가 있다. Phase 0 contracts, application scaffold, Phase 1 Creator capability, completion evidence, repository governance와 accepted Gate ownership을 포함한다. Completion gate는 `dev`에 merge됐으며 `dev → main` PR은 만들지 않았다.
...
[PR #15](https://github.com/xxh3898/form-dock/pull/15)는 Gate 3 release eligibility와 Gate 4 Production activation ownership을 분리한 ADR-0005를 `9d29f47165968b0308ea7b3e90c2183fe68cc48c`로 merge했다. Exact merge SHA의 `dev` push Validate [32262007298](https://github.com/xxh3898/form-dock/actions/runs/32262007298)는 세 job 모두 `success`다.

# 4. 최신 `dev` Regression Evidence

Run [32262007298](https://github.com/xxh3898/form-dock/actions/runs/32262007298)은 exact head `9d29f47165968b0308ea7b3e90c2183fe68cc48c`에 연결된 `dev` branch의 실제 `push` event다.
...
|---|---|---|
| full release diff validation | `PASS — evidence candidate` | [Phase 1 Main Release Evidence](phase-1-main-release-evidence.md)가 exact baseline, ancestry, final candidate counts, integration boundaries, runtime/schema/API scope와 exclusion audit를 기록한다 |
| ARM64 build | `PASS — evidence candidate` | [Phase 1 Main Release Evidence](phase-1-main-release-evidence.md)의 exact Hosted run이 existing API/Web Dockerfile를 native ARM64에서 build하고 두 image metadata를 `arm64/linux`로 검사했다 |
...
READY TO OPEN — evidence PR merge와 latest merged dev verification 이후 효력 발생.
                Recovery action execution과 Production activation은 Gate 4 work.
```

Evidence PR이 아직 merge되지 않은 동안 effective status는 `BLOCKED`다. Merge와 latest `dev` verification 이후에만 별도 Phase 1 `dev → main` Release Issue/PR을 열 수 있다. Phase 2 Product Issue와 Production recovery action은 계속 승인되지 않는다.
